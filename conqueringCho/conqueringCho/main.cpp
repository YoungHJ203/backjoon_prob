...
int min(int a = 0, int b = 0, int c = -1, int d = -1);
...
int main(){
	...
	for (int i = 0; i < testCase; i++) {
		numSec=0;					// the half number of sections
		numPlat=0;				// the number of platoon members
		minPlat=0;				// minimum number of platoons
		count=0;				// checking how many cases are possible

		// get the number of sections and number of platoon members
		cin >> numSec >> numPlat;

		vector<vector<int>>	enemy(2,vector<int> (numSec));	// the number of enemy in each section
		...
		if(enemy[0][0]+enemy[0][numSec -1]<=numPlat){
			minPlat=min(dp(numSec,numPlat,enemy,1),minPlat);
			count++;
		}

		if(enemy[1][0]+enemy[1][numSec -1]<= numPlat){
...
int dp(int N, int W, vector<vector<int>> &enemy, int checking){
		...
		for(i=1;i<N-1;i++){
				...
				if(i!=1 || (checking!=1 && checking!=2 && checking!=3))
					dt[1][3]=min(dt[1][3],dt[0][0]+(enemy[1][i-1]+enemy[1][i]<=W && enemy[0][i - 1] + enemy[0][i] <= W ?2:4));
		...
		if(i==N-1){
			...
			switch(checking){
				case 0:{	dt[1][0]=dt[0][3];
							dt[1][1]=min(dt[0][2]+(enemy[0][i-1]+enemy[0][i]<=W?1:2),dt[1][0]+1);
							dt[1][2]=min(dt[0][1]+(enemy[1][i-1]+enemy[1][i]<=W?1:2),dt[1][0]+1);
							dt[1][3]=min(dt[1][0]+(enemy[0][i]+enemy[1][i]<=W?1:2),dt[1][1]+1,dt[1][2]+1,dt[0][0]+ (enemy[0][i - 1] + enemy[0][i] <= W && enemy[1][i - 1] + enemy[1][i] <= W ? 2 : 4));
							return dt[1][3];}
